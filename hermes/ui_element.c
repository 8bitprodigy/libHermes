...
{
    int x = 0, y = 0;
    Hermes_Platform_get_screen_pos(&element->window->window, &x, &y);
...
void Hermes_ElementPaint(UIElement *element, UIPainter *painter)
{
    ...
    for (uintptr_t i = 0; i < element->childCount; i++) {
        ...
        Hermes_ElementPaint(element->children[i], painter);
    }
