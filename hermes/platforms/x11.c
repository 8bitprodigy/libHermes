...
void Hermes_Platform_CreateWindow(UIWindow *window, uint32_t flags, const char *cTitle, int _width,
                                 int _height)
{
    Hermes_Platform_X11 *platform = ui.platform;
    ...
    if (flags & UI_WINDOW_CENTER_IN_OWNER && window->owner) {
        int x = 0, y = 0;
        Hermes_Platform_get_screen_pos(&window->owner->window, &x, &y);
...
{
    Hermes_Platform_X11 *platform_x11 = ui.platform;
...
{
    Hermes_Platform_X11 *platform = calloc(1, sizeof(*platform));
    if (NULL == platform) {
        return NULL;
    }
