...
{
    const Hermes_Platform *platform = ui.platform;
    ...
    if (message == WM_CLOSE) {
        ...
        if (message == WM_SETFOCUS) {
            Hermes_Inspector_SetFocusedWindow(window);
...
{
    Hermes_Platform *platform = calloc(1, sizeof(*platform));
    if (NULL == platform) {
        return NULL;
    }
...
UIWindow *Hermes_Platform_CreateWindow(UIWindow *owner, uint32_t flags, const char *cTitle,
...
void Hermes_Platform_get_screen_pos(UIWindow *window, int *_x, int *_y)
{
    POINT p;
    p.x = 0;
    p.y = 0;
